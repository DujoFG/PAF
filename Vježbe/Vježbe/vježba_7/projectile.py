...
class Projectile:
    ...
    def kosi_hitac(self,dt):
        ...
        t=dt
        while self.y[i]>=0:
            self.aex.append(-np.sign(self.vx[i])*((self.ro*self.Cd*self.A)/(2*self.m0))*(self.vx[i])**2)
            self.vx.append(self.vx[i]+self.aex[i]*dt)
            self.x.append(self.x[i]+self.vx[i+1]*dt)
            self.aey.append(-9.81-np.sign(self.vy[i])*((self.ro*self.Cd*self.A)/(2*self.m0))*(self.vy[i])**2)
            self.vy.append(self.vy[i]+self.aey[i]*dt)
            self.y.append(self.y[i]+self.vy[i+1]*dt)  
            self.t.append(t)
            t+=dt
            i+=1
        return self.x,self.vx,self.aex,self.y,self.vy,self.aey,self.t
